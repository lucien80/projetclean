...
def analyze_usb():
    ...
    # Démonter tous les points de montage des clés USB
    subprocess.run(["umount", "/media/sd*"])

    # Supprimer tous les dossiers correspondants dans /media/sd*
    subprocess.run(["rm", "-rf", "/media/sd*"])

    # Afficher une alerte lorsque le démontage est terminé
    print("Les points de montage des clés USB ont été démontés et les dossiers ont été supprimés.")


def update_clamav_database():
    print("Updating ClamAV database...")
    result = subprocess.run(['freshclam'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

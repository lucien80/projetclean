...
def get_device_name(device_path):
    with open(f"{device_path}/device/model", "r") as f:
        return f.read().strip()
...
def analyze_usb():
    ...
    # Get the names of the connected USB drives
    device_names = [get_device_name(device) for device in devices]
    ...
    for dirpath, dirnames, filenames in tqdm.tqdm(os.walk(selected_device)):
